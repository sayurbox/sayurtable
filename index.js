...
class SayurTable extends Component {
  ...
  renderBody = ({loading, rows, columns, networkStatus}) => {
    if (loading) {
      return <StatusRow message={'Loading ...'} colSpan={columns.length + 1} />;
    } else if (networkStatus == 8) {
      return <StatusRow message={'Error ...'} colSpan={columns.length + 1} />;
    } else {
      if (!rows || rows.length == 0) {
        return <StatusRow message={'No Data'} colSpan={columns.length + 1} />;
      } else {
  ...
  renderSortIcons = ({key}) => {
    ...
    if (key == sortBy && desc) {
      return <i className="sort-by-desc pull-right" />;
    } else if (key == sortBy && !desc) {
      return <i className="sort-by-asc pull-right" />;
    } else {
      return (
    ...
    return (
      <div className="panel panel-default">
        ...
        <table
          ...
          <thead>
            <tr style={{backgroundColor: '#9CB93B', color: 'white'}}>
              <th>No</th>
              {this.renderColumnHeader({columns})}
            </tr>
          </thead>
          <tbody>
            {this.renderBody({loading, rows, columns, networkStatus})}
          </tbody>
        </table>
      </div>
    );
